...
def checkerboard(): #exercise 25
    #checkerboard recives no arguements
    #it draws a checker board
    #ouputs a checker board
    ...
    for height in range(HEIGHT):
        for width in range(WIDTH):
            if isBlack == True:
                my_graphics.square(x_cord, y_cord, sqaurewidth, 'black')
                x_cord = x_cord + sqaurewidth
                isBlack = False
            else:
                my_graphics.square(x_cord, y_cord, sqaurewidth, 'white')
                x_cord = x_cord + sqaurewidth
                isBlack = True
                
            x_cord = x_cord - (sqaurewidth * WIDTH)
            y_cord = y_cord - sqaurewidth
    turtle.done()
